...
class LiarsPokerQueryService {
  LiarsPokerQueryService({FirebaseDatabase? database})
  ...
  Stream<DatabaseEvent> watchPrivateHand({
    ...
    return _database.ref('rooms/$roomCode/game/private/$uid/hand').onValue;
  }

  /// 현재 턴
  Stream<DatabaseEvent> watchTurn(String roomCode) {
    return _database.ref('rooms/$roomCode/game/public/turnUid').onValue;
  }

  /// 플레이어 상태
  Stream<DatabaseEvent> watchPlayers(String roomCode) {
    return _database.ref('rooms/$roomCode/game/public/players').onValue;
  }

  /// 테이블 카드
  Stream<DatabaseEvent> watchTable(String roomCode) {
    return _database.ref('rooms/$roomCode/game/public/table').onValue;
  }
