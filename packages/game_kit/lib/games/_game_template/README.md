...
이 문서는 라이어스포커(`liars_poker`, 가장 완성된 참고 게임)와 파이널콜(`final_call`, 두 번째 참고 게임)을 실제로 조사해서 정리한, 새 게임을 만들 때 따라야 할 파일 구조·서비스/프로바이더 연결·화면 흐름·애니메이션·문구 컨벤션입니다. 복사해서 시작할 스캐폴드는 이 폴더(`_game_template/`)에 이미 있습니다 — `services/`, `screens/`, `example_game.dart`.
...
제공하고, 나머지 폴더는 위 트리에 맞춰 게임 요구사항에 따라 추가합니다. 플랫폼
공용 계약은 복사하지 않고 `package:game_contract/games/template_game.dart`를
import합니다. 새 게임은
독립 workspace package인 `packages/game_<my_game>/lib/games/<my_game>/`에 둡니다.

번들 게임의 에셋도 소유 패키지 안에서 같은 규칙을 따릅니다:
`packages/game_<my_game>/assets/games/<my_game>/{animations,images,sounds}`. Shorebird
패치로 추가하는 다운로드 게임은 pubspec asset을 두지 않고 원격 매니페스트를 씁니다.
...
모든 게임은 쓰기(Command, Cloud Functions)와 읽기(Query, Realtime Database 스트림)를 분리하고, `XService(command, query)` 파사드로 묶습니다. `_game_template/services/`가 이 모양의 스켈레톤입니다.

**두 서비스는 공용 베이스를 상속합니다. 직접 구현하지 마세요.**

| 베이스 | 물려받는 것 |
| --- | --- |
| [`GameCommandService`](../shared/services/game_command_service.dart) | callable 리전, `controllerCommandData` 첨부, 재전송 정책, 응답 Map 변환, `commandId()` 생성, `warmUpCommands()` 예열 |
| [`GameQueryService`](../shared/services/game_query_service.dart) | `publicRef`/`privateRef` 경로 조합, `watchPublicGame`, `readPublicGame`, `watchStatus`, `watchPrivatePlayer` |

```dart
class MyGameCommandService extends GameCommandService {
  MyGameCommandService({super.functions, super.retryPolicy});
  // 이 게임의 명령만 추가합니다.
}

class MyGameQueryService extends GameQueryService {
  MyGameQueryService({super.database});
  // 이 게임에만 있는 노드만 추가합니다. 경로는 publicRef/privateRef로 만듭니다.
}
```

지켜야 할 것:

- **RTDB 경로 문자열을 직접 만들지 않습니다.** `publicRef(roomCode, 'turnUid')`,
  `privateRef(roomCode: ..., uid: ..., child: 'hand')`를 씁니다. 게임마다 경로를
  손으로 조합하면 `game/public·private` 경계가 바뀔 때 한 게임만 빠뜨려도
  조용히 잘못된 경로를 구독하게 됩니다.
- **명령 오류를 자체 예외로 감싸지 않습니다.** `userErrorMessage`와 컨트롤러가
  `FirebaseFunctionsException`의 `code`와 서버가 내려준 한국어 `message`를
  직접 읽습니다. 포장하면 그 두 값이 가려져 일반 문구만 보입니다.
- `retryTransientFailure: true`는 **payload에 `commandId`가 있는 멱등 명령에만**
  줍니다. 없이 재전송하면 게임이 두 번 진행될 수 있습니다.
...
**컨트롤러 상태 설계 기준** (`game_contract`의 `games/template_game.dart` 문서 주석과 동일):
...
`GameRegistry.games`에 `<MyGame>Game()` 인스턴스 하나만 추가하면 앱 셸이 주입한
`GameCatalog`을 통해 아래가 자동으로 연결됩니다. 게임별 분기 코드를 플랫폼 쪽에 새로 추가하지 마세요:
...
- **에러/토스트**: 정중한 완료형 문장(`'게임에서 퇴장하지 못했습니다.'`, `'게임 정보를 불러오지 못했습니다.'`).
  **예외 원문을 문구에 끼우지 마세요.** `'...: $error'`처럼 쓰면 정상 퇴장 중 권한이 사라져 발생한
  `permission-denied`의 영문 원문과 네이티브 stack trace가 그대로 사용자에게 노출됩니다.
  구독·명령 오류는 `mosigame_core`의 `core/error/user_error_message.dart`에 있는
  `userErrorMessage(error, context: UserErrorContext.gameSubscription)`으로 변환하고,
  결과가 `null`이면 아무것도 표시하지 않습니다. 원본 예외는 `DevErrorLog`에만 남깁니다.
...
7. `lib/games/game_registry.dart`의 `GameRegistry.games`에 `const <MyGame>Game()`을 추가합니다 — 플랫폼 쪽 코드는 수정하지 않습니다.
   다운로드 게임은 `requiredAssetVersion`을 1 이상으로 재정의합니다. 앱 시작 때 영구
   캐시와 Firebase Storage source에 자동 등록되지만 다운로드는 시작되지 않습니다.
...
Shorebird patch로 처음 추가하는 게임은 패키지 `pubspec.yaml`에 `assets:`를 만들지 않고
`assets.gen.dart`도 생성하지 않습니다. 이미지와 사운드는 다음 핸들만 사용합니다.

```dart
const background = GameImage.remote(
  gameId: 'my_game',
  assetVersion: 1,
  logicalPath: 'images/background.webp',
);

final turnSound = GameAssetStore.remoteSoundReference(
  gameId: 'my_game',
  assetVersion: 1,
...
Firebase Storage에는 `game-assets/<gameId>/manifest.json`과
`game-assets/<gameId>/<assetVersion>/<logicalPath>`를 올립니다. 매니페스트의 gameId와
assetVersion은 게임 코드와 정확히 같아야 합니다. 다운로드 화면을 붙일 때 소유권을
확인한 뒤 `GameAssetStore.instance.downloadGame(gameId)`을 호출합니다. 등록이나 게임
진입 검사는 다운로드를 시작하지 않습니다.
