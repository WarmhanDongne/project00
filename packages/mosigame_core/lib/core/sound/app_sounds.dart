...
abstract final class AppSounds {
  ...
  static const dealing = 'packages/mosigame_core/assets/sounds/dealing.mp3';

  /// 벌칙 결과 도장이 프로필에 찍히는 순간 재생하는 효과음입니다.
  ///
  /// 도장이 내려오기 시작할 때가 아니라 실제로 닿는 순간(진행도 0.30)에
  /// 재생합니다.
  static const stamp = 'packages/mosigame_core/assets/sounds/stamp.mp3';

  /// 벌칙 룰렛의 레버를 끝까지 내렸을 때 한 번 재생하는 효과음입니다.
  ///
  /// 레버가 실제로 잠기는 순간에만 재생합니다. 임계점에 못 미쳐 손을 뗀 뒤
  /// 레버가 제자리로 돌아가는 경우에는 재생하지 않습니다.
  static const lever = 'packages/mosigame_core/assets/sounds/lever.mp3';
  ...
  static const timer = 'packages/mosigame_core/assets/sounds/timer.mp3';

  /// 벌칙 룰렛이 도는 동안 재생하는 긴 효과음입니다.
  ///
  /// 파일이 룰렛 회전(4초)보다 길어서, 재생만 하고 두면 연출이 끝난 뒤에도
  /// 소리가 남습니다. 반드시 정지 가능한 채널로 재생하세요.
  static const roulette = 'packages/mosigame_core/assets/sounds/roulette.mp3';
}
