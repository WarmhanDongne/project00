...
class _GamePreviewDialogState extends State<GamePreviewDialog> {
  ...
  Future<void> _startGame() async {
    ...
    try {
      await GameAssetStore.instance.prepareGame(widget.game.id);
    } catch (error, stack) {
      CrashReporting.recordError(error, stack, reason: '게임 에셋 준비');
      if (!mounted) return;
      setState(() => _isStarting = false);
      _showMessage(context, '게임 파일을 준비하지 못했습니다. 네트워크를 확인해 주세요.');
      return;
    }
    if (!mounted) return;
    if (!isGamePlayableOnThisBuild(
      widget.game,
      catalog: widget.roomProvider.gameCatalog,
    )) {
      setState(() => _isStarting = false);
      _showMessage(context, gameRequiresUpdateMessage);
      return;
