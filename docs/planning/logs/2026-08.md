...
현재 상태는 [작업 목록](../TASKS.md), 최종 완료 판정은
[완료 문서](../COMPLETED_TASKS.md)를 따른다. 본문은 태스크별·날짜순으로 관리하며
목차는 날짜별 탐색용 링크다. 기록일과 실제 테스트일이 다르거나 미제공이면 구분한다.

## 날짜별 목차

| 날짜 | 태스크와 기록 |
| --- | --- |
| 2026-08-30 | [세션·인증·방 정리 공통 구현/배포](#batch-0830), [개별 문제·완료 조건](#legacy-task-notes) |
| 2026-08-31 | [최초 사용자 결과·조사](#batch-0831-initial), [후속 구현·FULL·배포](#batch-0831-implementation), [최종 사용자 확인](#batch-0831-final), [기존 확인·출시 증빙](#legacy-confirmations), [문서 재분류·신규 작업 등록](#docs-workflow-01) |

## 기존 기록의 출처와 해석

- 2026-08-31 이관 기준: branch develop, HEAD 147833c4e1ece8d76e51435b31c767093ec3e0ec의
  기존 출시 전·후 문서. 과거 결과를 이번 변경에서 재실행하거나 재검증한 것은 아니다.
- 아래 기록의 '이번', '현재 HEAD', '이 문서', 파일명·상대 경로의 인라인 표기는
  원문 작성 당시를 뜻한다. 실제 Markdown 링크는 이동 위치에 맞게 조정했다.
- 과거 실패·BLOCKED·미검증에서 후속 통과로 바뀐 과정을 보존한다. 당시의 실패를 현재
  미해결 상태로 되돌리거나 통과 환경을 확대하지 않는다.
- 과거 production 접근·배포 승인·명령은 역사적 근거다. 이번 작업이나 향후 작업에
  대한 승인 또는 실행 지시가 아니다. 현재 Engineering Contract와 별도 승인을 따른다.

## SESSION-RELEASE-BATCH-01 — 기존 여러 태스크의 공통 실행 기록

이 ID는 이관 시 부여한 공통 기록 식별자다. 별도의 새 제품 작업이나 완료 판정이 아니다.
대상: SESSION-RECONNECT-01, SESSION-TIMER-01, CONTROLLER-RECOVERY-01,
LEAVE-CONSISTENCY-01, AUTH-BACK-01, FINISH-SYNC-01, NET-UI-01, ROOM-CLEANUP-01.
원문이 같은 실행에서 함께 다룬 원인 표·테스트·배포 근거를 태스크별로 복제하지 않는다.
각 태스크의 최종 변경·근거 연결은 완료 문서에서 확인한다.

<a id="batch-0830"></a>

### 2026-08-30 · 최초 구현·검증·지정 배포
...
<a id="batch-0831-initial"></a>

### 2026-08-31 · 최초 사용자 결과·코드 조사·수정 계획
...
- Galaxy S20+는 이번 테스트 이전 UT에서 화면이 깨지지 않았다는 사용자 확인이 있다.
  ...
  [출시 후 접근성 개선](../TASKS.md#accessibility-scale-01--글씨화면-확대-설정-대응)에 기록한다.
...
<a id="batch-0831-implementation"></a>

### 2026-08-31 · 후속 구현·FULL·지정 배포
...
<a id="batch-0831-final"></a>

### 2026-08-31 · 최종 사용자 세 시나리오 통과
...
우려는 [GAME-PAUSE-UX-01](../TASKS.md#game-pause-ux-01--단절-후-남은-시간-재개-안내)에
기록한다. winner 아래 화면은 사용자 표현인 `태블릿 오류 화면`을 보존하며 정확한 문구나
route를 추정하지 않는다. [WINNER-CONNECTION-LAYER-01](../TASKS.md#winner-connection-layer-01--winner-아래-태블릿-연결-안내-겹침)로
...
<a id="legacy-task-notes"></a>

## 기존 개별 태스크의 문제·완료 조건

아래는 2026-08-30 최초 기록과 08-31 후속 메모다. 원문에 개별 날짜가 없는 문장에
새 테스트일을 부여하지 않는다. 현재 판정은 완료 문서에 모은다.

<a id="session-reconnect-01"></a>

### SESSION-RECONNECT-01 · 2026-08-30~31 기존 문제·완료 조건
...
### AUTH-BACK-01 · 2026-08-30~31 기존 문제·완료 조건
...
<a id="finish-sync-01"></a>

### FINISH-SYNC-01 · 2026-08-30~31 기존 문제·완료 조건
...
### NET-UI-01 · 2026-08-30~31 기존 문제·완료 조건
...
<a id="room-cleanup-01"></a>

### ROOM-CLEANUP-01 · 2026-08-30~31 기존 문제·완료 조건
...
<a id="docs-workflow-01"></a>

## DOCS-WORKFLOW-01 — 작업·완료·기록 문서 재분류

### 2026-08-31 · 구조 결정과 신규 작업 등록

- 사용자와 합의: 작업/완료/기록으로 구분하고 출시 전 필수·권장·출시 후는 작업 분류로
  관리한다. 기록은 월별 파일·태스크별 본문·날짜별 목차를 사용한다.
- 기존 완료 8개와 미해결 방 생성 요청·출시 후 관찰 4개의 상태를 유지했다. 기존
  출시 증빙 보완은 RELEASE-EVIDENCE-01로 식별했다. 이번 대화의 필수·권장·출시 후
  요청과 핵심/전체 코드 조사·최적화·문서화 범위를 작업 목록에 추가했다.
- 신규 항목은 사용자 보고·요청이다. 비용 원인·보안 결함·UI 원인을 조사 완료로
  표현하지 않았다. 장르 태그 의미와 테스트 계정 용도는 요구사항 확인 상태로 남겼다.
- 기존 두 문서의 상세 근거를 새 위치에 이관하고 기존 파일을 제거했다. 문서 목차,
  에이전트 진입점과 관련 운영 문서 링크를 새 위치로 연결한다.
- 현재 구현 설명 문서는 이관 대상이 아니다. 코드 점검 뒤 DOCS-CORE-01과
  DOCS-DETAIL-01에서 내용을 보완한다. 이번에는 앱/Functions 코드, dependency,
  상태·데이터 계약, production 조회·계정 생성·배포·migration을 변경/실행하지 않는다.
- 시작 상태: branch develop, HEAD 147833c4e1ece8d76e51435b31c767093ec3e0ec, staged/unstaged/untracked 없음.
  Git이 열거한 tracked/unignored 파일 997개의 SHA256을 기록했다.
  전역 ignore 파일 읽기 권한 경고는 설정을 바꾸지 않고 보존했다.
- 이 문서 정리를 제품 점검·최적화 완료로 간주하지 않는다.

### 2026-08-31 · 이관 검증과 완료

| 실행 명령·검사 | status | exit code·근거 |
| --- | --- | --- |
| `git branch --show-current`, `git rev-parse HEAD`, `git status --short`, `git diff --cached --name-only` | PASS | 0. branch develop·HEAD 동일, staged 없음 |
| `git ls-files --cached --others --exclude-standard -z`와 Node SHA256 전후 대조 | PASS | 0. 기존 파일 997개 → 998개; 문서 4개 수정·2개 제거·3개 추가만 확인 |
| `node "$env:TEMP/mosigame-docs-migration-20260831-check.cjs"` | PASS | 0. 기존 출시 전 본문과 출시 후 상세 항목 503줄 보존, Markdown 파일 링크·앵커·줄 끝 공백·충돌 마커 검사 |
| `git diff --check` | PASS | 0. tracked diff 검사; 신규 문서는 위 별도 검사에 포함 |
| `.\tool\invoke_mosigame.ps1 validate --full` — 제한 환경 | BLOCKED | 실제 exit 1. CLI startup marker가 60초 내 관찰되지 않아 종료. 코드 테스트 실패와 구분 |
| `.\tool\invoke_mosigame.ps1 validate --full` — 승인된 실행 환경 | PASS | 0. startup 4,723ms; 7 단계 PASS, 실패·blocked 0. Flutter 643개, Functions 274개 통과. snapshot A/B 동일 |

- 변경 파일: `AGENTS.md`, `docs/README.md`, 운영 기술 참조와 실기기 체크리스트의
  연결 문구·링크. 추가: `TASKS.md`, `COMPLETED_TASKS.md`, 이 월별 기록. 제거: 기존
  `PRE_RELEASE_BLOCKERS.md`, `POST_LAUNCH_IMPROVEMENTS.md` 두 파일.
- 본문 보존 검사는 제목·공백·이동한 링크 경로를 제외하고 원래 의미 있는 각 줄을
  새 세 문서와 대조했다. 출시 후 문서의 중복된 관리 안내는 작업 목록의 관리 방법으로
  통합했다. 기존 상태·보류 이유·완료 조건·검증 한계는 유지했다.
- FULL 뒤 문서 연결 문구와 이 완료 결과만 갱신하고 Markdown·이관·변경 범위 및
  `git diff --check`를 다시 검사했다. 이후 앱/Functions 코드 수정은 없다.
- 전후 branch·HEAD는 동일하고 staged 없음. 최종 상태는 위 문서 변경만 있는 dirty
  tree다. 전역 ignore 읽기 경고는 기존 환경의 한계이며 비교는 Git이 열거한 범위다.
- 별도 targeted suite·구현 스킬·실기기·Firebase production 접근·계정 생성·배포·
  migration·stage·commit·push는 수행하지 않았다. 이번 작업은 Markdown 문서 정리다.
