...
- **등록일·상태:** 2026-08-31 사용자 전달 사항. 로컬 수정·자동 테스트 완료,
  production 배포·실기기 재현 전이므로 차단 유지. 기존 항목의 통과·차단
  해제와 별도로 관리하며, 앞선 FULL PASS나 배포·실기기 결과로 이 항목까지
  해결됐다고 판단하지 않는다.
- **목적:** 방은 생성됐지만 앱이 응답을 받지 못해 재시도할 때, 요청 기록으로 중복 방
  생성을 막는다. 중복 방지 자체는 필요한 동작이다.
- **현재 문제:** 방과 요청 기록을 따로 삭제하므로 중간에 실패하면 요청 기록만 남을
  수 있다는 사용자 보고다.
- **확인한 원인:** 방과 `roomCreateRequests/{controllerUid}/{creationOperationId}`를
  별도로 삭제해, 방 삭제 후 요청 기록 삭제가 실패하면 잔류할 수 있었다.
- **적용한 수정:** `/rooms/{roomCode}` 삭제 trigger
  `cleanupDeletedRoomCreationRequest`를 추가했다. 삭제된 방의 controller UID와
  operation ID가 안전한 식별자일 때만 해당 예약 경로를 멱등 삭제하고,
  trigger retry를 켜 일시 실패 뒤 재처리한다. 중복 방지 기록 구조는 유지한다.
- **자동 검증:** Functions build·lint·276개 테스트 PASS. 안전한 경로 생성,
  형식이 잘못된 UID/operation ID 거부, 중복 보유 게임 ID 정리를 포함한다.
- **남은 해제 조건:** 해당 trigger를 배포하고, 테스트 방에서 방 삭제 후
  예약 경로가 없어지는지와 재시도가 중복 방을 만들지 않는지 확인한다.
  기존 production 잔류 규모 조회·정리는 이 로컬 작업에서 수행하지 않았다.
